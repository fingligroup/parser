...
// part 4
class CORE_GorodvCom_Parser extends ParserCore implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    ...
    public function __construct()
    {
        $this->config = [
            ...
            'element' => [
                ...
                // (опционально)
                'element-image'       => 'header.entry-header a[href]',

                // css-селектор для цитаты
                // (если не заполнено, то по умолчанию берутся теги: blockquote и q)
                // (опционально)
                //                'element-quote'       => 'p strong',
                'element-quote'       => '',

                // игнорируемые css-селекторы (будут вырезаться из результата)
                // (можно несколько через запятую)
                // (опционально)
                'ignore-selectors'    => '.entry-content p:first-of-type, .wh-link',
