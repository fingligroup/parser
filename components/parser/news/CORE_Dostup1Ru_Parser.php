...
// part 4
class CORE_Dostup1Ru_Parser extends ParserCore implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;
    ...
    public function __construct()
    {
        $this->config = [
            ...
            'element' => [
                ...
                // (обязательный)
                'container'           => 'section.middle-content',
                ...
                // (можно несколько через запятую, если есть разные шаблоны новости)
                // (обязательный)
                'element-text'        => '.main-news-content',
                ...
                // css селектор для описания элемента
                // (опционально)
                'element-description' => '.main-news-content > p:first-child',

                // css селектор для получения картинки
                // !должен содержать конечный аттрибут src! (например: img.main-image[src])
                // (опционально)
                'element-image'       => '.main-news-img-container img[src]',
                ...
                // (опционально)
                'ignore-selectors'    => '.main-news-content > p:first-child, div.main-news-img-container, .main-news-tags',

                // css-селекторы которые будут вставлятся в начало текста новости element-text (селекторы ищутся от корня)
                // (опционально)
                'element-text-before' => 'div.main-news-container.main-news-container__orange > div.main-news-img-container',
            ]
        ];
